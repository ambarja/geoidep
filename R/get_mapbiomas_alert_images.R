...
get_mapbiomas_alert_images <- \(alert_ids, download_dir = NULL,
  ...
  cli::cli_inform(c(
    "Download summary:",
    "*" = paste("{success_count} images downloaded successfully", cli::symbol$tick),
    if (failed_count > 0) "!" = paste("{failed_count} downloads failed", cli::symbol$cross),
    if (skipped_count > 0) "i" = paste("{skipped_count} images skipped (already exist)", cli::symbol$info)
  ))
